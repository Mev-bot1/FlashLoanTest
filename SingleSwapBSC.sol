...
contract FlashLoanBSC {
    ...
    address private immutable WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    address private immutable CAKE = 0x8d008B313C1d6C7fE2982F62d32Da7507cF43551;
    ...
    constructor( ) {
        swapRouter = ISwapRouter(pancakeRouterV3T);
    }
    ...
    function swapExactInputSingle(uint256 amountIn) external returns (uint256 amountOut) {
        ...
        TransferHelper.safeTransferFrom(CAKE, msg.sender, address(this), amountIn);

        // Approve the router to spend DAI.
        TransferHelper.safeApprove(CAKE, address(swapRouter), amountIn);
...
//function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut);
    // function How (uint _amountIn) public view returns (uint[] memory amounts){
         
    //     address[] memory path = new address[](2);
    //     path[0] = CAKE;
    //     path[1] = WBNB;

    //     amounts = pancakeRouterV3.exactInputSingle(params);(_amountIn,path);
    // }


    function getQuote(uint _amountIn) public   returns (uint[] memory amounts){
        ...
        TransferHelper.safeApprove(CAKE,pancakeRouterV2T,_amountIn);
        amounts = pancakeRouterV2.swapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline); (amountIn, amountOutMin, path, to, deadline);
    }
